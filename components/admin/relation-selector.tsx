...
export const RelationSelector = <T extends Relation>({
  ...
  const suggestRelations = suggestedIds
    ? relations.filter((r) => suggestedIds.includes(r.id))
    : [];
  const [suggestedRelations, setSuggestedRelations] =
    useState<T[]>(suggestRelations);
  const selectedRelations = relations?.filter(({ id }) =>
  ...
  const getDisplayRelations = (relations: T[], sort = false): Relation[] => {
    const sortedRelations =
      sort && sortFunction ? [...relations].sort(sortFunction) : relations;
    return sortedRelations.map((relation) =>
      mapFunction ? mapFunction(relation) : relation,
    );
  };
            ...
            <CommandList className="min-w-72 w-(--radix-popper-anchor-width)">
              ...
              <CommandGroup>
                {getDisplayRelations(relations, true).map((relation) => {
                  ...
                  const isSuggested = suggestedRelations.find(
                    (r) => r.id === relation.id,
                  );
                  ...
                  return (
                    <CommandItem
                      ...
                      onSelect={() => {
                        ...
                        setSuggestedRelations((rel) =>
                          rel.filter(({ id }) => id !== relation.id),
                        );
                      }}
                    >
                      <input
