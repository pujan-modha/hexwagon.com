...
export const AdsPicker = ({
  ...
  const [isSidebarTargetingEnabled, setIsSidebarTargetingEnabled] = useState(false)
  ...
  const sidebarSelection = selections.find(selection => selection.type === "Sidebar")
  const hasSidebarSelection = Boolean(sidebarSelection?.dateRange?.from && sidebarSelection?.dateRange?.to)
  ...
  const themeRelations = useMemo<TargetRelation[]>(
    () => targetThemes.map(theme => ({ id: theme.slug, name: theme.name, faviconUrl: theme.faviconUrl })),
    [targetThemes],
  )

  const platformRelations = useMemo<TargetRelation[]>(
    () => targetPlatforms.map(platform => ({ id: platform.slug, name: platform.name, faviconUrl: platform.faviconUrl })),
    [targetPlatforms],
  )
  ...
  const effectiveDiscountedPrice = (price?.discountedPrice ?? 0) + targetingSurcharge
  ...
  const handleCheckout = () => {
    if (isSidebarTargetingEnabled && targetingCount === 0) {
      toast.error("Select at least one theme or platform to enable targeted sidebar ads.")
      return
    }
  ...
  const isCheckoutDisabled = !hasSelections || isPending || (isSidebarTargetingEnabled && targetingCount === 0)
      ...
      <div className="flex flex-wrap overflow-clip">
        {adSpots.map(adSpot => (
          <AdsCalendar
            ...
            ignoreBooked={adSpot.type === "Sidebar" && isSidebarTargetingEnabled}
        ...
        {hasSidebarSelection && (
          <div className="grid gap-3 border-t p-4">
            <Stack className="w-full justify-between">
              <div>
                <strong className="text-sm font-medium text-foreground">Targeted sidebar ads</strong>
                <p className="text-xs text-muted-foreground">
                  Pick specific themes/platforms. Matching pages show targeted ads only.
                </p>
              </div>
              ...
              <Stack size="sm">
                <Checkbox
                  ...
                  onCheckedChange={checked => setIsSidebarTargetingEnabled(checked === true)}
                />
                <label htmlFor="enable-sidebar-targeting" className="text-sm text-foreground">
                  Enable
                </label>
              </Stack>
            </Stack>
            ...
            {isSidebarTargetingEnabled && (
              <div className="grid gap-3 md:grid-cols-2">
                <div className="grid gap-2">
                  <p className="text-xs uppercase tracking-widest text-muted-foreground">Themes</p>
                  <RelationSelector
                ...
                <div className="grid gap-2">
                  <p className="text-xs uppercase tracking-widest text-muted-foreground">Platforms</p>
                  <RelationSelector
            ...
            {isSidebarTargetingEnabled && (
              <Note className="text-xs">
                {targetingCount} {plur("target", targetingCount)} × ${targetingUnitPrice.toFixed(2)} = ${targetingSurcharge.toFixed(2)} extra
              </Note>
            )}
          </div>
        )}
        ...
        {price ? (
          <>
            <Stack size="sm" className="mr-auto">
              <Note>Total:</Note>
              <Price price={effectiveDiscountedPrice} fullPrice={effectiveFullPrice} />
            </Stack>

            {price.discountPercentage > 0 && (
              <Tooltip tooltip={`Discount applied based on the order value. Max ${maxDiscountPercentage}% off.`}>
                <Badge
                  size="lg"
                  variant="outline"
