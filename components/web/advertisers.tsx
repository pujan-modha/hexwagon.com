...
export const Advertisers = async ({
  className,
  ...props
}: ComponentProps<"div">) => {
  ...
  const longTermAds = ads.filter((ad: Ad) => {
    const durationInMs =
      new Date(ad.endsAt).getTime() - new Date(ad.startsAt).getTime();
  ...
  const advertisers = Array.from(
    new Map(longTermAds.map((ad: Ad) => [ad.name, ad])).values(),
  );

  return (
    <div
      className={cx("flex flex-col items-center text-center gap-6", className)}
      {...props}
    >
