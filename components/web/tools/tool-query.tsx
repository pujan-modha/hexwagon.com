    ...
    <div className="grid gap-4">
      {ports.map(port => (
        <Card key={port.id} className="p-5">
          <Link href={`/themes/${port.theme.slug}/${port.platform.slug}/${port.id}`} className="font-semibold">
            {port.name ?? `${port.theme.name} for ${port.platform.name}`}
          </Link>

          {port.description && <p className="mt-2 text-sm text-muted-foreground">{port.description}</p>}
        </Card>
      ))}
    </div>
  )
