...
const ManagedThemeCard = ({ theme }: { theme: MaintainedTheme }) => {
  ...
  const [pendingOfficialPortId, setPendingOfficialPortId] = useState<string | null>(null)
    ...
    <Card className="gap-5">
      <CardHeader className="justify-between">
        ...
        <div className="text-xs text-muted-foreground">
          ...
          {theme._count.maintainers === 1 ? "" : "s"} · {theme._count.ports} published port
          {theme._count.ports === 1 ? "" : "s"}
        </div>
      </CardHeader>
          ...
          groupedPorts.map(ports => {
            ...
            return (
              <div key={platform.slug} className="grid gap-2.5">
                <div className="text-xs font-medium text-muted-foreground">{platform.name}</div>
                ...
                <div className="grid gap-2">
                  {ports.map(port => (
                    ...
                    >
                      <Link href={`/themes/${theme.slug}/${platform.slug}/${port.id}`}>
                      ...
                      {port.isOfficial ? (
                        ...
                        <Button
                          ...
                          variant="secondary"
                          disabled={
                            markOfficialAction.isPending && pendingOfficialPortId !== port.id
                          }
                          isPending={
                            markOfficialAction.isPending && pendingOfficialPortId === port.id
                          }
...
export const DashboardMaintainedThemesSection = ({ themes }: MaintainedThemesSectionProps) => {
  return (
    <section className="mt-10 space-y-4">
      <div>
        <h2 className="text-xl font-semibold tracking-tight">Maintained Themes</h2>
      ...
      {themes.length > 0 ? (
        ...
        <Note>
          You are not assigned as a maintainer for any theme yet. Claim a theme to manage it here.
        </Note>
      )}
    </section>
  )
