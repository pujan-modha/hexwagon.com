...
export const getColumns = (): ColumnDef<AdRow>[] => {
  return [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex min-w-0 flex-col gap-1">
            ...
            <span className="truncate text-[11px] text-muted-foreground">{ad.websiteUrl}</span>
          </div>
        )
      },
    },
    {
      ...
      cell: ({ row }) => <Badge variant="outline">{String(row.getValue("source"))}</Badge>,
    },
    {
      accessorKey: "type",
      header: ({ column }) => <DataTableColumnHeader column={column} title="Type" />,
      cell: ({ row }) => <Badge variant="outline">{String(row.getValue("type"))}</Badge>,
    },
    {
      accessorKey: "status",
      header: ({ column }) => <DataTableColumnHeader column={column} title="Status" />,
      cell: ({ row }) => <Badge variant="outline">{String(row.getValue("status"))}</Badge>,
    },
    {
      accessorKey: "rotation",
      accessorFn: row => (row.status === "Approved" ? "Active" : "Inactive"),
      header: ({ column }) => <DataTableColumnHeader column={column} title="Rotation" />,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const clickCount = Number(row.getValue("clickCount") ?? 0)

        return (
          <span className="text-sm font-medium tabular-nums">{clickCount.toLocaleString()}</span>
        )
      },
    },
    {
