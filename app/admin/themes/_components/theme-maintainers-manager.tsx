...
import { assignThemeMaintainer, removeThemeMaintainer } from "~/server/admin/themes/actions"
...
export const ThemeMaintainersManager = ({ themeId, maintainers }: ThemeMaintainersManagerProps) => {
  ...
  return (
    <div className="rounded-md border p-4 grid gap-3 col-span-full">
      <Stack className="justify-between">
        ...
        <span className="text-xs text-muted-foreground">{maintainers.length} assigned</span>
      </Stack>

      {maintainers.length ? (
        <div className="grid gap-2">
          {maintainers.map(maintainer => (
            <Stack key={maintainer.userId} className="justify-between rounded border px-3 py-2">
