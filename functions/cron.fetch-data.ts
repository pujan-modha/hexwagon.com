...
export const fetchData = inngest.createFunction(
  ...
  async ({ step, db }) => {
    const [ports, themes, platforms] = await Promise.all([
      step.run("fetch-ports", async () => {
        return await db.port.findMany({
          where: {
            status: { in: [PortStatus.Published, PortStatus.Scheduled] },
          },
        });
      }),
    ...
    await step.run("post-on-socials", async () => {
      const publishedPorts = ports.filter(
        (port) => port.status === PortStatus.Published,
      );
      const port =
        publishedPorts[Math.floor(Math.random() * publishedPorts.length)];
