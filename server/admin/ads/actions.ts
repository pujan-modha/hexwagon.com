...
import {
  ...
  notifyAdvertiserOfAdChangesRequested,
  notifyAdvertiserOfAdLive,
  notifyAdvertiserOfAdRejected,
} from "~/lib/notifications"
import { adminProcedure } from "~/lib/safe-actions"
import { db } from "~/services/db"
import { cancelPayPalSubscription, refundPayPalPayment } from "~/services/paypal"
...
export const rejectAd = adminProcedure
  ...
  .handler(async ({ input: { adId, reason } }) => {
    ...
    if (existingAd.billingProvider === "PayPal" && existingAd.billingSubscriptionId) {
      await cancelPayPalSubscription({
        subscriptionId: existingAd.billingSubscriptionId,
        reason: reason || "Campaign rejected by admin.",
      })
    }
    ...
    if (existingAd.paidAt && existingAd.billingLastPaymentId && !existingAd.refundedAt) {
      const refund = await refundPayPalPayment(existingAd.billingLastPaymentId)

      refundId = refund.id
      refundAmountCents = existingAd.priceCents
    }
    ...
    const ad = await db.ad.update({
      ...
      data: {
        ...
        billingRefundId: refundId ?? existingAd.billingRefundId,
      },
    })
...
export const requestAdChanges = adminProcedure
  .createServerAction()
  .input(rejectAdSchema)
  .handler(async ({ input: { adId, reason } }) => {
    const existingAd = await db.ad.findUniqueOrThrow({ where: { id: adId } })

    if (existingAd.status === adStatus.Rejected || existingAd.status === adStatus.Cancelled) {
      throw new Error("Cannot request changes for rejected or cancelled ads.")
    }

    const ad = await db.ad.update({
      where: { id: adId },
      data: {
        status: adStatus.PendingEdit,
        adminNote: reason,
        approvedAt: null,
        rejectedAt: null,
        cancelledAt: null,
    ...
    after(async () => {
      await notifyAdvertiserOfAdChangesRequested(ad)
    })
...
export const setAdminAdActive = adminProcedure
  ...
  .handler(async ({ input: { id, isActive } }) => {
    ...
    const isAdminManaged =
      !existingAd.billingProvider &&
      !existingAd.billingCheckoutReferenceId &&
      !existingAd.billingLastPaymentId &&
      !existingAd.billingSubscriptionId
