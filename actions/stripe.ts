...
import {
  createAdPackageDraftToken,
  verifyAdPackageDraftToken,
} from "~/lib/ad-package-draft-token";
import {
...
const resolveAdFaviconUrl = async ({
  ...
  return (await tryCatch(uploadFavicon(websiteHost, `ads/${websiteHost}`)))
    .data;
};
...
export const createStripeAdPackageCheckout = createServerAction()
  ...
  .handler(async ({ input: { draftToken, ...adDetailsInput } }) => {
    ...
    const cyclePricing =
      draft.billingCycle === AdBillingCycle.Monthly
        ? pricing.monthly
        : pricing.weekly;

    const targetCount = draft.themeIds.length + draft.platformIds.length;
    const targetingTotalPriceCents =
      cyclePricing.targetUnitPriceCents * targetCount;
    const totalAmountCents =
      cyclePricing.discountedPriceCents + targetingTotalPriceCents;
...
export const createAdFromCheckout = createServerAction()
  ...
  .handler(
    async ({ input: { sessionId, sessionToken, ...adDetailsInput } }) => {
      const { faviconFile, ...adDetails } = adDetailsInput;
