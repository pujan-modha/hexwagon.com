...
type NewsletterInput = z.input<typeof newsletterSchema>
...
export const subscribeEmailToNewsletter = async (
  ...
  const { value: email, captcha, ...input } = newsletterSchema.parse(rawInput)
  const ip = await getIP()
  const rateLimitKey = `newsletter:${ip}`

  // Rate limiting check
  if (await isRateLimited(rateLimitKey, "newsletter", { bypass: options.bypassRateLimit })) {
...
export const subscribeToNewsletter = createServerAction()
  ...
  .handler(async ({ input }) => subscribeEmailToNewsletter(input))
